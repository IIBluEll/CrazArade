...
public class Re_PhotonManager : MonoBehaviourPunCallbacks
{
    ...
    public void ConnectRoomLobby()
    {
        id_UI.SetActive(false);
    ...
    public override void OnJoinedRoom()
    {
        ...
        {
            PhotonNetwork.LoadLevel("WaitRoom_HL");
        }
    }
        ...
        foreach (var room in roomList)
        {
            ...
            else
            {
                if (roomdic.ContainsKey(room.Name) == false)
                {
                    ...
                    GameObject _room = Instantiate(roomPrefab, Re_RoomPrefeb_Manager.instance.InstansiateRoom());
                    _room.GetComponent<Re_RoomBtn_Info>().RoomInfo = room;
                    roomdic.Add(room.Name, _room);
                }
                else
                {
                    roomdic.TryGetValue(room.Name, out tempRoom);
                    tempRoom.GetComponent<Re_RoomBtn_Info>().RoomInfo = room;
